...
import { itemChanged, cleaItem } from "@/Redux/Reducers/AdminFormsEditReducer";
...
function Edit_Item({ id, name }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (!id) {
            toast.error(
                "Item Id is not defined, please update the item from the items list"
            );
            return;
        }
        ...
        const edit_formData = new FormData();
        if (image) {
            edit_formData.append("file", image);
        }
        edit_formData.append("itemTitle", itemName);
        edit_formData.append("itemPrice", itemPrice);
        edit_formData.append("itemId", id);
        edit_formData.append("unitId", itemUnit);
        edit_formData.append("categoryId", itemCategory);
